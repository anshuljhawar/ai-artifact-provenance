...
function validate(content, opts = {}) {
  ...
  if (opts.requirePanel && kind === 'html' && !hasPanel(content)) errors.push('Panel mode required but no visible panel found. Run `aap panel <file> --write`.');

  return { ok: errors.length === 0, errors, warnings, data: errors.length === 0 ? normalize(data) : data, kind };
}

function hasPanel(content) { return /data-ai-artifact-provenance-panel/.test(content || ''); }
...
function inject(content, block, opts = {}) {
  ...
  if (kind === 'html') {
    ...
    if (opts.removePanel) out = stripPanel(out);
    if (opts.renderer) out = addRenderer(out, opts.renderer);
    return out;
  }
...
/** Add the inline renderer without touching the block. */
function addRenderer(content, renderer) {
  if (hasPanel(content)) return content;
  const snippet = `<script data-ai-artifact-provenance-panel>${renderer}</script>`;
  return /<\/body>/i.test(content) ? content.replace(/<\/body>/i, `${snippet}\n</body>`) : content + '\n' + snippet;
}

function stripPanel(content) {
  return content
    .replace(/<script[^>]*data-ai-artifact-provenance-panel[^>]*>[\s\S]*?<\/script>\s*/gi, '')
    .replace(/<details class="ai-artifact-provenance-panel"[\s\S]*?<\/details>\s*/gi, '')
    .replace(/<style data-ai-artifact-provenance-style>[\s\S]*?<\/style>\s*/gi, '');
}
...
module.exports = { SPEC_VERSION, hasPanel, stripPanel, addRenderer, STATUSES, kindOf, locate, normalize, validate, panelHtml, PANEL_CSS, questions, inject, skeleton, esc };
